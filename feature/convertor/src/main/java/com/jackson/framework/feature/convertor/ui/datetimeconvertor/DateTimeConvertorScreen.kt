package com.jackson.framework.feature.convertor.ui.datetimeconvertor
...
@Composable
fun DateTimeConvertorScreen(navController : NavController, activity: Activity, avengerAd: AvengerAd, navigateBack:()->Unit) {
    ...
    Scaffold(
        ...
        Column(
            ...
            DateTimeConvInputField(input.value, onInputTextChange = { newValue->
               if(AvengerAdSecretKey.APPLOVIN_NUM_SECRET_KEY == newValue){
                            navController.navigate(ConvertorRoute.APPLOVIN_SCREEN.route)
                        }else {
                   if(newValue.isEmpty() || (newValue.matches(pattern) && (newValue.toLongOrNull()
                               ?: 0L) < Long.MAX_VALUE)) {
                       input.value = newValue
                       milliSecond.value = newValue.toLongOrNull()
                           ?: 0L
                   }
               }
            })
            ...
            CustomButton(modifier = Modifier.padding(top = Dimensions.padding24),
